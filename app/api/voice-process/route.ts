...
function getVoicePrompt(fieldName: string, language: string): string {
  ...
  const fieldLabel = fieldName.replace(/_/g, ' ').replace(/([A-Z])/g, ' $1').trim();
...
function getBackendResponse(
...
): { response: string; isConfirmed: boolean; action?: string; nextField?: string } {
  const langCode = (language && typeof language === 'string') ? language.split('-')[0] : 'en';
  ...
  const responses: Record<string, Record<string, any>> = {
    name: {
      ...
      ta: {
        ...
        error: 'தயவுசெய்து குறைந்தபட்சம் 2 எழுத்துக்கள் கொண்ட செல்லுபடியாகும் பெயரை வழங்கவும்.',
      },
      ml: {
    ...
    email: {
      ...
      te: {
        ...
        error: 'దయచేసి చెల్లుబాటైన ఇమెయిల్ చిరునామా ఇవ్వండి (ఉదా: name@example.com).',
      },
      ta: {
        ...
        error: 'தயவுசெய்து சரியான மின்னஞ்சல் முகவரியை வழங்கவும் (உ.ம: name@example.com).',
      },
      ml: {
      ...
      bn: {
        ...
        error: 'দয়া করে একটি বৈধ ইমেল ঠিকানা প্রদান করুন (যেমন name@example.com)।',
      },
      gu: {
      ...
      or: {
        ...
        error: 'ଦୟାକରି ଏକ ବୈଧ ଇମେଲ୍ ଠିକଣା ପ୍ରଦାନ କରନ୍ତୁ (ଉ.ଦା: name@example.com)।',
      },
      pa: {
      ...
      ur: {
        ...
        error: 'براہ کرم ایک درست ای میل پتہ فراہم کریں (مثلاً name@example.com)۔',
      },
    },
    phone: {
      en: {
    ...
    address: {
      ...
      ta: {
        ...
        error: 'தயவுசெய்து குறைந்தபட்சம் 10 எழுத்துக்கள் கொண்ட முழுமையான முகவரியை வழங்கவும்.',
      },
      ml: {
      ...
      kn: {
        ...
        error: 'ದಯವಿಟ್ಟು ಕನಿಷ್ಠ 10 ಅಕ್ಷರಗಳನ್ನು ಹೊಂದಿರುವ ಸಂಪೂರ್ಣ ವಿಳಾಸವನ್ನು ನೀಡಿ.',
      },
      mr: {
      ...
      bn: {
        ...
        error: 'দয়া করে কমপক্ষে 10 টি অক্ষর সহ একটি সম্পূর্ণ ঠিকানা প্রদান করুন।',
      },
      gu: {
    ...
    gender: {
      ...
      te: {
        ...
        error: 'దయచేసి చెల్లుబాటైన లింగం ఇవ్వండి (పురుషుడు, స్త్రీ, లేదా ఇతరులు).',
      },
      ta: {
      ...
      ml: {
        ...
        error: 'കൃപയാ സാധുവായ ലിംഗം നല്കുക (പുരുഷൻ, സ്ത്രീ, അല്ലെങ്കിൽ മറ്റുള്ളവ).',
      },
      kn: {
      ...
      bn: {
        ...
        error: 'দয়া করে একটি বৈধ লিঙ্গ প্রদান করুন (পুরুষ, মহিলা, বা অন্যান্য)।',
      },
      gu: {
      ...
      or: {
        ...
        error: 'ଦୟାକରି ଏକ ବୈଧ ଲିଙ୍ଗ ପ୍ରଦାନ କରନ୍ତୁ (ପୁରୁଷ, ସ୍ତ୍ରୀ, କିମ୍ବା ଅନ୍ୟ)।',
      },
      pa: {
    ...
    dob: {
      ...
      ta: {
        ...
        error: 'தயவுசெய்து சரியான பிறந்த தேதியை வழங்கவும் (உ.ம:ஆ 15 ஆகஸ்ட் 1990).',
      },
      ml: {
  ...
  const fileFields = ['poi_file', 'poa_file', 'dob_file', 'poi_doc', 'poa_doc', 'dob_doc', 'doc_type', 'citizen_proof', 'address_proof', 'hospital_doc', 'parent_doc', 'caste_proof_file', 'income_proof_file', 'income_proof_doc', 'family_id_doc', 'birth_cert_file', 'id_proof_file', 'address_proof_file'];
  ...
  if (!trimmedTranscript || trimmedTranscript.length === 0) {
    ...
    isValid = false;
  } else if (fieldName === 'phone' && !/^\d{10}$/.test(trimmedTranscript.replace(/\D/g, ''))) {
    isValid = false;
    ...
    const validGenders = ['male', 'female', 'other', 'man', 'woman', 'm', 'f', 'masculino', 'femenino', 'पुरुष', 'महिला', 'अन्य'];
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!transcript || !fieldName) {
      return NextResponse.json(
        { success: false, error: 'transcript and fieldName are required' } as VoiceProcessResponse,
        { status: 400 }
      );
    }

    // Get backend response logic
    const backendResponse = getBackendResponse(transcript, language, fieldName, context);
