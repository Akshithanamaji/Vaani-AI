...
// Compact data keys (used when QR embeds minimal data to stay scannable)
interface CompactData {
    ...
    t?: number;    // timestamp
    st?: string;   // status
    // Full format keys (backwards compat)
    serviceName?: string;
    userDetails?: Record<string, string>;
    submittedAt?: number;
    status?: string;
...
/** Decode either compact or full base64-JSON embedded in the QR data param */
function decodeDataParam(dataParam: string): SubmittedService | null {
    try {
        const base64 = dataParam.replace(/ /g, '+');
        // Try UTF-8 safe decode first, then plain atob
        let jsonStr: string;
        try {
        ...
        const decoded: CompactData = JSON.parse(jsonStr);
        if (!decoded?.id) return null;

        // Handle compact format (keys: sn, n, p, t, st)
        if (decoded.sn !== undefined) {
            const userDetails: Record<string, string> = {};
            if (decoded.n) userDetails['Name'] = decoded.n;
            if (decoded.p) userDetails['Phone'] = decoded.p;
            return {
                id: decoded.id,
                serviceName: decoded.sn || 'Government Service',
                submittedAt: decoded.t || Date.now(),
                status: (decoded.st as SubmissionStatus) || 'submitted',
                userDetails,
                statusHistory: [],
            };
        }
        ...
        if (decoded.serviceName) {
            return {
                ...
                submittedAt: decoded.submittedAt || Date.now(),
                status: (decoded.status as SubmissionStatus) || 'submitted',
...
export default function SubmissionDetailsContent() {
    ...
    const [source, setSource] = useState<'qr' | 'api' | 'local'>('api');

    useEffect(() => {
        const fetchSubmission = async () => {
            try {
                // ── PRIORITY 1: ?data= param embedded in QR URL ─────────────────────
                // The QR code encodes all data in base64 — works on any device/network.
                ...
                if (dataParam) {
                    ...
                    if (decoded) {
                        ...
                        // Still try to get full details from API in background
                        try {
                            const res = await fetch(`/api/submissions/${id}?viewer=user`);
                            if (res.ok) {
                        ...
                        } catch { /* silent — QR data is already shown */ }
                        return;
                    }
                }

                // ── PRIORITY 2: Fetch from server API ────────────────────────────────
                try {
                    ...
                    // server unreachable (phone off-network) — continue to localStorage
                }

                // ── PRIORITY 3: localStorage (same browser only) ─────────────────────
                try {
                    const stored = localStorage.getItem('vaani_submissions');
                    if (stored) {
                        const submissions = JSON.parse(stored);
                        const found = submissions.find((s: SubmittedService) => s.id === id);
                        if (found) {
                            setSubmission(found);
                            setSource('local');
                            setLoading(false);
                ...
                setError('Submission not found. Please check the QR code or link.');
            } catch (e) {
                console.error('Error fetching submission:', e);
                setError('Could not load submission. Please check your connection.');
            } finally {
                setLoading(false);
            }
        };
                            ...
                            <div className="p-8">
                                ...
                                {source === 'qr' && (
                                    <div className="mb-4 p-3 bg-blue-50 border border-blue-100 rounded-xl flex items-center gap-2">
                                        <span className="text-blue-600 text-xs font-semibold">ℹ️ Showing data from QR code. Full details available when connected to the same network.</span>
                                    </div>
                                )}
